...
def pass_epoch(
    ...
    for i_batch, (x, y) in enumerate(loader):
        ...
        metrics_batch = {}
        for metric_name, metric_fn in batch_metrics.items():  # acc: training acc
            metrics_batch[metric_name] = metric_fn(y_pred, y).detach().cpu()
            metrics[metric_name] = metrics.get(metric_name, 0) + metrics_batch[metric_name]
            
        if writer is not None and model.training:  # train
    ...
    if writer is not None and not model.training:  # evaluate
