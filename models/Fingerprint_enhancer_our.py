...
class FingerprintImageEnhancer_tensor(object):
    ...
    def rot_img(self, x, theta, dtype):
        ...
        grid = F.affine_grid(rot_mat, x.size()).type(dtype).cuda()
        x = F.grid_sample(x, grid)
        return x

    def __normalise(self, img, mean, std):
        # normed = (img - np.mean(img)) / (np.std(img))
        normed = (img - torch.mean(img)) / (torch.std(img))
        return (normed)
    ...
    def __ridge_segment(self, img):
        ...
        padded_img = torch.zeros((new_rows, new_cols))
        stddevim = torch.zeros((new_rows, new_cols))
        ...
        for i in range(0, new_rows, self.ridge_segment_blksze):
            for j in range(0, new_cols, self.ridge_segment_blksze):
                ...
                stddevim[i:i + self.ridge_segment_blksze][:, j:j + self.ridge_segment_blksze] = torch.std(block) * torch.ones(block.shape)

        stddevim = stddevim[0:rows][:, 0:cols]
        self._mask = stddevim > self.ridge_segment_thresh
        # mean_val = np.mean(im[self._mask])
        mean_val = torch.mean(im[self._mask])
        # std_val = np.std(im[self._mask])
        std_val = torch.std(im[self._mask])
        self._normim = (im - mean_val) / (std_val)
        # cv2.imshow('normalised_image', self._normim)
    ...
    def __ridge_orient(self):
        ...
        # todo here padding may not be 'same'
        # if fx.shape[0] % 2 == 0:  # 偶数
        #     padding_x = fx.shape[0] // 2
        # else:
        #     padding_x = (fx.shape[0] - 1) // 2
        # if fy.shape[0] % 2 == 0:  # 偶数
        #     padding_y = fy.shape[0] // 2
        # else:
        #     padding_y = (fy.shape[0] - 1) // 2
        ...
        # todo here weight和fx,fy精度不同，还不够精确
        fx = torch.FloatTensor(fx).unsqueeze(0).unsqueeze(0)
        fy = torch.FloatTensor(fy).unsqueeze(0).unsqueeze(0)
        self._normim = self._normim.unsqueeze(0).unsqueeze(0)
        Cx.weight, Cy.weight = Parameter(fx.cuda(), requires_grad=False), Parameter(fy.cuda(), requires_grad=False)
        Cx.bias, Cy.bias = Parameter(torch.FloatTensor([0]).cuda(), requires_grad=False), Parameter(torch.FloatTensor([0]).cuda(), requires_grad=False)
        Gx = Cx(self._normim)
        Gy = Cy(self._normim)
        self._normim = self._normim.squeeze()
        ...
        # if f.shape[0] % 2 == 0:  # 偶数
        #     padding_f = f.shape[0] // 2
        # else:
        #     padding_f = (f.shape[0] - 1) // 2
        ...
        self._orientim = torch.tensor(np.pi) / 2 + torch.atan2(sin2theta, cos2theta) / 2
    ...
    def __ridge_freq(self):
        ...
        self._mask = self._mask.detach().cpu().numpy()
        self._freq = freq * self._mask
        freq_1d = np.reshape(self._freq, (1, rows * cols))

        # adapt min_mave_length
        while np.all(freq_1d == 0):
    ...
    def __ridge_freq_(self):
        ...
        self._freq = freq * self._mask
        freq_1d = torch.reshape(self._freq, (1, rows * cols))

        # adapt min_mave_length
        while torch.all(freq_1d == 0):
            self.min_wave_length -= 1
            if self.min_wave_length <= 0:
                print('bad min_wave_length!')
                exit(0)
            # calculate frequency again
            for r in range(0, rows - self.ridge_freq_blksze, self.ridge_freq_blksze):
                for c in range(0, cols - self.ridge_freq_blksze, self.ridge_freq_blksze):
        ...
        # self._mean_freq = np.mean(non_zero_elems_in_freq)
        self._mean_freq = torch.mean(non_zero_elems_in_freq)
        # self._median_freq = np.median(non_zero_elems_in_freq)  # does not work properly
        self._median_freq = torch.median(non_zero_elems_in_freq)
    ...
    def __frequest(self, blkim, blkor):
        ...
        blkim = blkim.detach().cpu().numpy()
        blkor = blkor.detach().cpu().numpy()
    ...
    def __frequest_(self, blkim, blkor):
        # FREQEST - Estimate fingerprint ridge frequency within image block
        #
        # Function to estimate the fingerprint ridge frequency within a small block
        # of a fingerprint image.  This function is used by RIDGEFREQ
        ...
        # sinorient = np.mean(np.sin(2 * blkor))
        # orient = math.atan2(sinorient, cosorient) / 2
        cosorient = torch.mean(torch.cos(2 * blkor))
        sinorient = torch.mean(torch.sin(2 * blkor))
        orient = math.atan2(sinorient, cosorient) / 2


        # Rotate the image block so that the ridges are vertical

        # ROT_mat = cv2.getRotationMatrix2D((cols/2,rows/2),orient/np.pi*180 + 90,1) no
        # rotim = cv2.warpAffine(im,ROT_mat,(cols,rows)) no
        # rotim = scipy.ndimage.rotate(blkim, orient / np.pi * 180 + 90, axes=(1, 0), reshape=False, order=3,
        #                              mode='nearest')    # here important
        # todo To replace rotate
        blkim = blkim.unsqueeze(0).unsqueeze(0)  # shape: [1, C, H, W]
        rotim = self.rot_img(blkim, orient / np.pi * 180 + 90, dtype=torch.FloatTensor)
        blkim = blkim.squeeze()
        rotim = rotim.squeeze()
        ...
        # proj = np.sum(rotim, axis=0)
        proj = torch.sum(rotim, axis=0)
        proj = proj.detach().cpu().numpy()  # todo here to detach!!!!
        dilation = scipy.ndimage.grey_dilation(proj, self.ridge_freq_windsze, structure=np.ones(self.ridge_freq_windsze))
        ...
        # rows_maxind, cols_maxind = np.shape(maxind[0])
        cols_maxind = maxind[0].shape[0]
        ...
        if (cols_maxind < 2):
            # return(np.zeros(blkim.shape))
            return (torch.zeros(blkim.shape))
        else:
            NoOfPeaks = cols_maxind
            waveLength = (maxind[0][cols_maxind - 1] - maxind[0][0]) / (NoOfPeaks - 1)
            if waveLength >= self.min_wave_length and waveLength <= self.max_wave_length:
                # return(1 / np.double(waveLength) * np.ones(blkim.shape))
                return (1 / waveLength * torch.ones(blkim.shape))
            else:
                # return(np.zeros(blkim.shape))
                return (torch.zeros(blkim.shape))
    ...
    def __ridge_filter(self):
        ...
        im = self._normim.double()
        rows, cols = im.shape
        # newim = np.zeros((rows, cols))
        newim = torch.zeros((rows, cols))
        ...
        # non_zero_elems_in_freq = torch.round((non_zero_elems_in_freq * 100)).double() / 100

        unfreq = np.unique(non_zero_elems_in_freq)
        # unfreq = torch.unique(non_zero_elems_in_freq)
        ...
        # sze = torch.round(3 * max(sigmax, sigmay)).int()

        x, y = np.meshgrid(np.linspace(-sze, sze, (2 * sze + 1)), np.linspace(-sze, sze, (2 * sze + 1)))
        # x, y = torch.meshgrid(torch.linspace(-sze, sze, (2 * sze + 1)), torch.linspace(-sze, sze, (2 * sze + 1)))
        ...
        # reffilter = torch.exp(-(((torch.pow(x, 2)) / (sigmax * sigmax) + (torch.pow(y, 2)) / (sigmay * sigmay)))) * torch.cos(
        #     2 * torch.tensor(np.pi) * unfreq[0] * x)
        ...
        gabor_filter = torch.zeros((angleRange, filt_rows, filt_cols)).cuda()

        # reffilter = reffilter.unsqueeze(0).unsqueeze(0)  # shape: [1, C, H, W]
        for o in range(0, angleRange):
            ...
            # rot_filt = self.rot_img(reffilter.cuda(), -(o * self.angleInc + 90), dtype=torch.FloatTensor)
            gabor_filter[o] = torch.from_numpy(rot_filt).cuda()
        ...
        # validr, validc = torch.where(temp)
        ...
        # finalind = torch.where(final_temp)

        # Convert orientation matrix values from radians to an index value
        # that corresponds to round(degrees/angleInc)

        maxorientindex = np.round(180 / self.angleInc)
        orientindex = np.round(self._orientim.detach().cpu().numpy() / np.pi * 180 / self.angleInc)
        # maxorientindex = torch.tensor(np.round(180 / self.angleInc))
        # orientindex = torch.round(self._orientim / torch.tensor(np.pi) * 180 / self.angleInc)
        ...
        finalind_cols = finalind[0].shape[0]
        sze = int(sze)
        for k in range(0, finalind_cols):
            ...
            g = gabor_filter[int(orientindex[r][c]) - 1]

            multi = img_block * g

            newim[r][c] = torch.sum(multi)

        # newim = newim.numpy()
        # cv2.imshow('newim', newim)
        # self._binim = newim < self.ridge_filter_thresh  # todo delete important
        self._binim = newim  # todo return a grey image, expected as tensor
    ...
    def enhance(self, img, resize=True):
        ...
        # self._binim = 255 * self._binim.astype('uint8')  #todo delete important
        return(self._binim)
...
def enhance_Fingerprint(img, resize=False, ridge_segment_blksze=16, ridge_segment_thresh=0.1, gradient_sigma=1,
                        block_sigma=5, orient_smooth_sigma=5,
                        ridge_freq_blksze=38, ridge_freq_windsze=5, min_wave_length=10, max_wave_length=15, kx=0.7,
                        ky=0.7, angleInc=3.0, ridge_filter_thresh=-2):

    image_enhancer = FingerprintImageEnhancer_tensor(ridge_segment_blksze, ridge_segment_thresh, gradient_sigma, block_sigma, orient_smooth_sigma,
