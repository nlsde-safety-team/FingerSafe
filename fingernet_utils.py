...
def gausslabel(length=180, stride=2):
    ...
    return gaussian_pdf[delta.astype(np.int)]
...
def draw_minutiae(image, minutiae, fname, r=25):
    image = np.squeeze(image)
    fig = plt.figure()
    plt.imshow(image,cmap='gray')
    plt.plot(minutiae[:, 0], minutiae[:, 1], 'rs', fillstyle='full', markersize=6, linewidth=5)
    for x, y, o in minutiae:
        plt.plot([x, x+r*np.cos(o)], [y, y+r*np.sin(o)], 'r-', linewidth=3)
