...
## Prerequisites
* Request dataset here http://www4.comp.polyu.edu.hk/~csajaykr/fingerprint.htm (We are not able to share it here because of license requirement)
* Color the image using this paper: Gray2colornet: Transfer more colors from reference image
* Put the data in ./datasets
* Train a fingerprint recognition model using `identification.py` or `verification.py`, put the trained model in ./best_models

---

## How to run this code
### train a model
Make sure the dataset is organized as: 4 train images in 'train' subfolder, 2 test images in 'test' subfolders (should be the default of HKPolyU); each class have a folder with two subfolders as mentioned.
...
digital world data preparation:
* input: `./datasets/final/veri_test`，the organization is 68 classes * 6 images per class
* output: `./datasets/final/veri_sample_XXX`，the organization is 68 classes * 6 images per class

physical world data preparation
* input: `./datasets/physical_square/evaluation`，the organization is 20 classes * 10 images per class
* output: `./datasets/physical_square/sample_XXX`，the organization is 20 classes * 10 images per class

##### training stage：

digital world data preparation
* input: `./datasets/final/train`，the organization is 268 classes * 6 images per class
* output: `./datasets/final/perturb_XXX`，the organization is 268 classes * 6 images per class
...
### Other operations
##### Preprocessing new adversarial images（convert=1~3，for MHS，HG，Frangi，convert=4 for JPEG compression）
...
##### Generate the map that organize the paths to verification: 
...
##### Preparing data for training stage:
...
###### Grey images input (after preprocessing)：
